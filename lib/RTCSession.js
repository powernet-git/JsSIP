...
module.exports = class RTCSession extends EventEmitter
{
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    return Promise.resolve()
      ...
      .then(() =>
      {
        ...
        const hasIceServers = connection.getConfiguration().iceServers?.length > 0;
        
        const iceTransport = constraints?.forceUseStunResponse && hasIceServers ? connection?.getSenders()?.[0]?.transport?.iceTransport : null;

        const forceUseStunResponse = constraints?.forceUseStunResponse && iceTransport != null;
        const forceUseStunResponseAndIsOutgoing = forceUseStunResponse && type === 'offer';
        const forceUseStunResponseAndIsIngoing = forceUseStunResponse && type === 'answer';
        
        let publicIceCandidate;
          ...
          (connection.iceGatheringState === 'gathering' && this._iceReady))
        {
          ...
          if (forceUseStunResponse) {
            e.sdp = this._correctSdpIfForcingStun(e.sdp, forceUseStunResponseAndIsIngoing, forceUseStunResponseAndIsOutgoing, iceTransport, publicIceCandidate)
          }
          logger.debug('emit "sdp"');
        ...
        return new Promise((resolve) =>
        {
          ...
          let selectedCandidatePairChangeListener;
          ...
          const ready = () =>
          {
            ...
            connection.removeEventListener('selectedcandidatepairchange', selectedCandidatePairChangeListener);
          ...
          if (!forceUseStunResponse || forceUseStunResponseAndIsOutgoing)
          {
            connection.addEventListener('icecandidate', iceCandidateListener = (event) =>
            {
              ...
              const isStunResponse = event.candidate.type === 'srflx' && event.candidate.relatedAddress !== null && event.candidate.relatedPort !== null;
          ...
          {
            iceTransport.addEventListener('selectedcandidatepairchange', selectedCandidatePairChangeListener = () =>
            {
              this.emit('selectedcandidatepairchange', { selectedCandidatePair: iceTransport.getSelectedCandidatePair() });
              if (forceUseStunResponse)
              {
                ready();
              }
  ...
  _substituteIceCandidatesWithLocalCandidate(sdp, localCandidate)
  {
    ...
    newLines.push(`a=${localCandidate}`);

    let newSdp = newLines.filter((line) => line.trim() !== '').join('\r\n');

    newSdp += '\r\n'; // Original SDP has trailing \r\n, so we recreate it

    return newSdp;
    // const e = { originator: 'local', type: type, sdp: connection.localDescription.sdp };
  }
  _correctSdpIfForcingStun(sdp, forceUseStunResponseAndIsIngoing, forceUseStunResponseAndIsOutgoing, iceTransport, publicIceCandidate)
  {
    if (forceUseStunResponseAndIsIngoing) {
      return this._substituteIceCandidatesWithLocalCandidate(sdp, iceTransport.getSelectedCandidatePair().local.candidate) ?? sdp;
    } else if (forceUseStunResponseAndIsOutgoing) {
      const bestCandidate = publicIceCandidate ?? iceTransport?.getSelectedCandidatePair()?.local;
      if (bestCandidate?.candidate) {
        return this._substituteIceCandidatesWithLocalCandidate(sdp, bestCandidate.candidate) ?? sdp;
      }
    }
    return sdp;
  }
